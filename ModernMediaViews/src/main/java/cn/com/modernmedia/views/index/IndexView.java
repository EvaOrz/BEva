...
public class IndexView extends BaseView implements FetchEntryListener {
    ...
    public void setTopMenuSelect(String tagname) {
        topMenu.setSelectedItemForChild(tagname);
    }
    ...
    public void setData(Entry entry) {
        ...
        if (entry instanceof TagArticleList) indexListView.setData((TagArticleList) entry, null);
        currTag = LIST;
    ...
    public boolean doGoBack() {
        return indexViewPager.getVisibility() == View.VISIBLE && indexViewPager.doGoBack();
    }

    public void doGoTop() {
        if (indexViewPager != null) {
            TagIndexListView listView = indexViewPager.getCurrListView();
            if (listView != null) listView.scrollToTop();
        }
    }
}
