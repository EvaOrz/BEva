...
public class IssueItemView extends RelativeLayout {
    ...
    private void init() {
        this.addView(LayoutInflater.from(mContext).inflate(R.layout.issue_list_item_single, null));
    ...
    public void setData(final TagInfo item, IssueListAdapter adapter) {
        ...
        if (ParseUtil.listNotNull(issueProperty.getPictureList())) {
            CommonApplication.finalBitmap.display(image, issueProperty.getPictureList().get(0));
        }
        title.setText(issueProperty.getTitle());
        desc.setText(getDate(issueProperty.getStartTime(), issueProperty.getEndTime()));
        bar.setVisibility(pro.getStatus() == DownloadProcessView.LOADING ? View.VISIBLE : View.GONE);
        // 设置下载进度
        if (adapter != null) {
     ...
     * @return
     */
    protected String getDate(long startTime, long endTime) {
        return DateFormatTool.format2(startTime * 1000, endTime * 1000, "yyyy-MM-dd", "MM-dd", "~");
    }

    /**
     * 计算图片大小
     *
     * @param viewWidth view的大小
     */
    private void computerImageSize(int viewWidth) {
        imgWidth = viewWidth - mContext.getResources().getDimensionPixelSize(R.dimen.issue_pic_marginleft);
        imgWidth -= mContext.getResources().getDimensionPixelSize(R.dimen.issue_pic_marginright);
        imgWidth -= 2 * mContext.getResources().getDimension(R.dimen.issue_pic_spacing);
        imgWidth /= 3;
    ...
    public void setImgSize(int viewWidth) {
        ...
        pro.setSize(mContext.getResources().getDimensionPixelSize(R.dimen.issue_process_size));
        // textview的大小与图片相同
