...
public class ChildCatItem extends RelativeLayout implements ChildCatClickListener, LoadCallBack {
    ...
    public ChildCatItem(Context context, View defaultHeadView, TagInfo tagInfo, TagInfoList childInfoList, ChildIndexView childIndexView) {
        super(context);
        mContext = context;
    ...
    private void init(View defaultHeadView) {
        addView(LayoutInflater.from(mContext).inflate(R.layout.solo_index, null));
        ...
        if (defaultHeadView != null) listView.addHeaderView(defaultHeadView);
    ...
    private void getArticleList(final boolean refresh, final boolean loadMore) {
        if (!refresh && !loadMore) showLoading();
        String _top = loadMore ? top : "";
        OperateController.getInstance(mContext).getTagArticles(mContext, tagInfo, _top, "", null, new FetchEntryListener() {
    ...
    private void getCatIndex(final boolean refresh, final boolean loadMore) {
        String _top = loadMore ? top : "";
        TagArticleList t = !refresh && TextUtils.equals(_articleList.getViewbygroup(), TagArticleList.BY_INPUTTIME) ? _articleList : null;
        OperateController.getInstance(mContext).getTagIndex(tagInfo, _top, "", t, new FetchEntryListener() {
     ...
     * 获取所有订阅栏目前5篇文章
     */
    private void getSubscribeTopArticle(final TagArticleList dArticleList, final boolean loadMore) {
