...
public class BaseIndexAdapter extends CheckScrollAdapter<ArticleItem> {
        ...
        synchronized (list) {
            for (ArticleItem item : list) {
                if (item.isShowTitleBar()) {
                    ...
                    // 专栏的栏目聚合模板 style=8
                    if (TextUtils.equals(item.getFromtagname(), "cat_244")) {
                        titleItem.getPosition().setStyle(103);
                        titleItem.setZhuanlanAuthor(item.getZhuanlanAuthor());
                    } else {
                        titleItem.getPosition().setStyle(100);
                ...
                // 3.0.0 6为广告模板 3.5.1 101为广告模板
                if (item.isAdv() != 0 && item.getPosition().getStyle() == 101) {
                    if (!ParseUtil.mapContainsKey(template.getList().getMap(), 101)) {
    ...
    public void addDesc(int position) {
        try {
            if (!descList.contains(Integer.valueOf(position))) descList.add(position);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void downImage(ArticleItem item, View image, boolean usePicture) {
        if (usePicture) {
            if (ParseUtil.listNotNull(item.getPicList()))
                CommonApplication.finalBitmap.display(image, item.getPicList().get(0).getUrl());
        } else {
            if (ParseUtil.listNotNull(item.getThumbList()))
                CommonApplication.finalBitmap.display(image, item.getThumbList().get(0).getUrl());
        }
    }
