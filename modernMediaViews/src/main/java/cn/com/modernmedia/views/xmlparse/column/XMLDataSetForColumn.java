...
public class XMLDataSetForColumn extends BaseXMLDataSet {
    ...
    public XMLDataSetForColumn(Context context, HashMap<String, View> map, List<View> clickViewList, List<View> ninePatchViewList, ColumnAdapter adapter) {
        super(context, map, clickViewList, ninePatchViewList);
        this.adapter = adapter;
    }

    public void setData(TagInfo info, int position) {
        if (map == null || map.isEmpty() || info == null || adapter == null) return;
    ...
    private void title(TagInfo info, int id, int position) {
        ...
        if (select) {
            if (title.getTag(R.id.select_color) instanceof String) {
                title.setTextColor(Color.parseColor(title.getTag(R.id.select_color).toString()));
            }
        } else {
            if (title.getTag(R.id.unselect_color) instanceof String) {
                title.setTextColor(Color.parseColor(title.getTag(R.id.unselect_color).toString()));
            }
        }
    }
    ...
    private void eTitle(TagInfo info, int id) {
        if (!map.containsKey(FunctionColumn.E_NAME) || id != 0) return;
    ...
    private void img(TagInfo info, int id) {
        ...
        if (id == 1 || id == 2) {
            if (ConstData.isGoodLife()) {
                icon.setBackgroundColor(id == 1 ? Color.parseColor("#70472C") : Color.parseColor("#6BCDFF"));
            } else {
    ...
    private void subscriptImg(int id) {
        if (!map.containsKey(FunctionColumn.SUBSCRIPT_IMG)) return;
    ...
    private void subscrip(int id) {
        if (!map.containsKey(FunctionColumn.SUBSCRIPT_FRAME)) return;
        View view = map.get(FunctionColumn.SUBSCRIPT_FRAME);
        if (id == 3 || id == 4) {
            view.setVisibility(View.VISIBLE);
            if (id == 4) view.setBackgroundColor(Color.TRANSPARENT);
        } else {
    ...
    private void divider(int id, int position) {
        if (!map.containsKey(FunctionColumn.DIVIDER)) return;
        View view = map.get(FunctionColumn.DIVIDER);
        view.setVisibility(View.VISIBLE);
        if (id == 4) {
            view.setVisibility(View.GONE);
        } else if (ConstData.getAppId() == 20 && AppValue.appInfo.getHaveSubscribe() == 1 && SlateApplication.mConfig.getHas_subscribe() == 1) {
    ...
    private void newDot(TagInfo info, int position) {
        if (!map.containsKey(FunctionColumn.NEW_IMG)) return;
        ...
        if (position > 0) {
            ...
            Map<String, ArrayList<Integer>> map = CommonApplication.lastestArticleId.getUnReadedArticles();
            String tagName = info.getTagName();
            if (ParseUtil.mapContainsKey(map, tagName) && map.get(tagName) != null && map.get(tagName).size() > 0) {
                view.setVisibility(View.VISIBLE);
            }
        }
    }
    ...
    private void click(final TagInfo item, final int position) {
        ...
        if (mContext instanceof CommonMainActivity) {
            ...
            //            if (CommonApplication.mConfig.getHas_single_view() != 1 || position != 0)
            //                V.setIndexTitle(mContext, item);
            //            ((CommonMainActivity) mContext).getScrollView().IndexClick();
            //            if (CommonApplication.mConfig.getHas_single_view() == 1 && position == 0 && clickFirstColumnListener != null) {
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    if (CommonApplication.mConfig.getHas_single_view() == 1) {
                        ...
                        //						} else {
                        ((CommonMainActivity) mContext).clickItemIfPager(item.getTagName(), false);
                        //						}
                        return;
                    }
