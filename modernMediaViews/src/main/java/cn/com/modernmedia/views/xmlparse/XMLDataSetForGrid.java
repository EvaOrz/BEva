...
public class XMLDataSetForGrid extends BaseXMLDataSet {
    ...
    public XMLDataSetForGrid(Context context, HashMap<String, List<GridItem>> gridMap, List<GridItem> clickViewList, List<View> ninePatchViewList, BaseIndexAdapter adapter) {
     ...
     *
     */
    public void setData(List<ArticleItem> list, int position, int oneLineCount, ArticleType articleType) {
        if (gridMap == null || gridMap.isEmpty() || !ParseUtil.listNotNull(list) || adapter == null)
            return;
        this.list = list;
        ...
        if (ParseUtil.listNotNull(clickViewList)) {
            for (GridItem gridItem : clickViewList) {
                ...
                if (list.size() > _p) registerClick(gridItem.getView(), list.get(_p), articleType);
            }
        }
    }
    ...
    private void frameContent() {
        if (!gridMap.containsKey(FunctionXML.FRAME_CONTENT)) return;
        List<GridItem> gridList = gridMap.get(FunctionXML.FRAME_CONTENT);
        if (!ParseUtil.listNotNull(gridList)) return;
        for (GridItem gridItem : gridList) {
            int _p = position * oneLineCount + gridItem.getIndex();
            gridItem.getView().setVisibility(list.size() > _p ? View.VISIBLE : View.INVISIBLE);
        }
    }
    ...
    private void titleBar() {
        if (!gridMap.containsKey(FunctionXML.TITLEBAR)) return;
        List<GridItem> gridList = gridMap.get(FunctionXML.TITLEBAR);
        if (!ParseUtil.listNotNull(gridList)) return;
        for (GridItem gridItem : gridList) {
            int _p = position * oneLineCount + gridItem.getIndex();
            if (list.size() <= _p) continue;
            ...
            if (ParseUtil.mapContainsKey(DataHelper.columnColorMap, item.getGroupname())) {
                view.setBackgroundColor(DataHelper.columnColorMap.get(item.getGroupname()));
            }
            if (view instanceof TextView) {
                if (ParseUtil.mapContainsKey(DataHelper.columnTitleMap, item.getGroupname())) {
                    ((TextView) view).setText(DataHelper.columnTitleMap.get(item.getGroupname()));
                }
            }
        }
    }
    ...
    private void titlebarDate() {
        ...
        for (GridItem gridItem : gridList) {
            ...
            if (item.isShowTitleBar()) {
                ...
                textView.setText(DateFormatTool.format(item.getInputtime() * 1000L, view.getTag(R.id.date_format).toString()));
            } else {
     ...
     */
    private void img() {
        if (!gridMap.containsKey(FunctionXML.IMAGE)) return;
     ...
     */
    private void title() {
        if (!gridMap.containsKey(FunctionXML.TITLE)) return;
     ...
     */
    private void desc() {
        if (!gridMap.containsKey(FunctionXML.DESC)) return;
    ...
    private void outline() {
        if (!gridMap.containsKey(FunctionXML.OUTLINE)) return;
     ...
     */
    private void tag() {
        if (!gridMap.containsKey(FunctionXML.TAG)) return;
    ...
    private void subTitle() {
        if (!gridMap.containsKey(FunctionXML.SUB_TITLE)) return;
    ...
    private void createUser() {
        if (!gridMap.containsKey(FunctionXML.CREATE_USER)) return;
    ...
    private void modifyUser() {
        if (!gridMap.containsKey(FunctionXML.MODIFY_USER)) return;
     ...
     */
    private void date() {
        if (!gridMap.containsKey(FunctionXML.DATE)) return;
     ...
     */
    private void fav() {
        if (!gridMap.containsKey(FunctionXML.FAV)) return;
     ...
     * @param view
     */
    private void registerClick(final View view, final ArticleItem item, final ArticleType articleType) {
        view.setOnClickListener(new OnClickListener() {
