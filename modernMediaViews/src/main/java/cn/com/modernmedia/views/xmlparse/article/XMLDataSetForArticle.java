...
public class XMLDataSetForArticle extends BaseXMLDataSet {
    ...
    public XMLDataSetForArticle(Context context, HashMap<String, View> map, List<View> clickViewList, List<View> ninePatchViewList) {
    ...
    protected void onClick(View view, ArticleItem item, ArticleType articleType) {
        if (!(view.getTag(R.id.click) instanceof String)) return;
    ...
    public View getNavBar() {
        ...
        if (map.containsKey(FunctionIndexNav.NAV_BAR)) return map.get(FunctionIndexNav.NAV_BAR);
        return null;
    }
    ...
    protected void fav() {
        ...
        if (ConstData.getInitialAppId() == 20) WeeklyLogEvent.logAndroidAddFavouriteCount(mContext);
    }
     ...
     * @param needFav 是否隐藏
     */
    public void hideKeyButton(boolean needFav, boolean needFont, boolean needShare, boolean needComment) {
        Set<String> keySets = map.keySet();
        for (String key : keySets) {
            View view = map.get(key);
            if (FunctionXML.FAV.equals(key)) {
                if (needFav) {
                    view.setVisibility(View.VISIBLE);
                } else {
                    view.setVisibility(View.GONE);
                }
            } else if (FunctionArticle.SHARE_IMG.equals(key)) {
                if (needShare) {
                    view.setVisibility(View.VISIBLE);
                } else {
                    view.setVisibility(View.GONE);
                }
            } else if (FunctionArticle.FONT_IMG.equals(key)) {
                if (needFont) {
                    view.setVisibility(View.VISIBLE);
                } else {
                    view.setVisibility(View.GONE);
                }
            } else if (FunctionArticle.ACRTICLE_CARD.equals(key)) {
                if (needComment) {
                    view.setVisibility(View.VISIBLE);
                } else {
                    view.setVisibility(View.GONE);
    ...
    public void showSettings(int duration) {
        if (!map.containsKey(FunctionArticle.SETTINGS)) return;
    ...
    public boolean isShowingSettings() {
        if (!map.containsKey(FunctionArticle.SETTINGS)) return false;
        View view = map.get(FunctionArticle.SETTINGS);
