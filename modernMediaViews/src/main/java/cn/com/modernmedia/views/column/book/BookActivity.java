...
public class BookActivity extends BaseActivity implements OnClickListener {
    ...
    private void initView() {
        ...
        if (ConstData.getAppId() == 20) {
            findViewById(R.id.select_column_title_frame).setBackgroundColor(getResources().getColor(R.color.column_head));
        ...
        bookingGridView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (ischeck) {
                    ...
                    Intent i = new Intent(BookActivity.this, BookColumnActivity.class);
    ...
    private void book() {
        ...
        for (TagInfo t : myAdapter.getTagsList()) {// 遍历已订阅列表
            if (t.getIsFix() != 1) {// 3.1.4 固定栏目要删除
                SubscribeColumn sub = new SubscribeColumn(t.getTagName(), null, 0);
                log += t.getTagName() + ",";
                cs.add(sub);
            }
        }
        ...
        if (SlateDataHelper.getUserLoginInfo(this) != null) {
            ...
            OperateController.getInstance(this).saveSubscribeColumnList(Tools.getUid(this), SlateDataHelper.getToken(this), cs, new FetchEntryListener() {
    ...
    public void isLogined() {
        if (SlateDataHelper.getUserLoginInfo(this) == null) {
            ...
            if (ischeck) {// 提交订阅
                ...
                for (int i = 0; i < myTags.size(); i++) {
                    if (myTags.get(i).getIsFix() != 1) myTags.get(i).setCheck(true);
                }
                myAdapter.notifyDataSetChanged();
            }
            ischeck = !ischeck;
