...
public class TopMenuHorizontalScrollView extends RelativeLayout {
    ...
    private void init() {
        view = (LinearLayout) LayoutInflater.from(mContext).inflate(R.layout.top_menu, null);
        addView(view);
        if (ConstData.getAppId() == 20) {// 设置iweekly背景颜色
            view.setBackgroundColor(Color.parseColor("#323232"));
        }
        scrollView = (HorizontalScrollView) view.findViewById(R.id.book_horizantal_scrollview);
        ...
        addView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mContext instanceof Activity)
                    ((Activity) mContext).startActivityForResult(i, CommonMainActivity.BOOK_ACTIVITY);
            }
        });
        columnView = (ImageView) view.findViewById(R.id.subscribe_column);
        if (ConstData.getAppId() == 1) {// 商业周刊3.3.0 版本
            ...
            columnView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    ((CommonMainActivity) mContext).getScrollView().clickButton(true);
                }
            });
        } else {
            columnView.setVisibility(View.GONE);
        }
    ...
    public void setData(List<TagInfo> tagInfos) {
        if (tagInfos == null || tagInfos.size() < 1) return;
        ...
        for (int i = 0; i < tagInfos.size(); i++) {
            if (!tagInfos.get(i).getTagName().equals("cat_191")) { // 去除视野
                ...
                if (tagInfo.getColumnProperty().getNoMenuBar() == 1) {
                    ...
                    cLayout.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
                ...
                cLayout.setOnClickListener(new OnClickListener() {
                    ...
                    @Override
                    public void onClick(View v) {
                        clickItem((TagInfo) v.getTag(), position);
                    }
                });
                if (ConstData.getAppId() == 20) {
                    ...
                    textChild.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
                }
    ...
    public void setSelectedItemForChild(String currTagName) {
        ...
        if (mContext instanceof CommonMainActivity)
            screenHalf = ((CommonMainActivity) mContext).getWindowManager().getDefaultDisplay().getWidth() / 2 - 44 * 2 - 20;
