...
public class TagIndexListView implements LoadCallBack, OnScrollListener {
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
			if (findFirstFullScreenItemView() == null) {
				helper.stopVideo();
			}
		} else {
			// 暂停列表和焦点图视频
			helper.pauseVideo(false);
			if (headView instanceof IndexViewHead) {
				((IndexViewHead) headView).getHelper().pauseVideo(false);
			}
		}
	}
	 ...
	 * @return
	 */
	private ArticleItem findFirstFullScreenItemView() {
		ArticleItem result = null;
		boolean containHeadView = false;// 判断焦点图是否滑出屏幕
		boolean hasFindVideo = false;// 是否已经找到可播放视频
		for (int i = 0; i < listView.getChildCount(); i++) {
			View child = listView.getChildAt(i);

			if (child instanceof IndexViewHead) {
				containHeadView = true;
			}
			...
			if (child instanceof IndexViewHead) {
				// 焦点图
				boolean headPlay = ((IndexViewHead) child).findVideo();
				if (headPlay) {
					// 让焦点图放
					hasFindVideo = true;
					helper.stopVideo();
			...
			if (!(child.getTag(R.id.adapter_article) instanceof ArticleItem)) {
				continue;
			}
			final ArticleItem item = (ArticleItem) child
					.getTag(R.id.adapter_article);
			...
			if (child.getBottom() > ViewsApplication.height) {
				continue;
			}
			if (!(child.getTag() instanceof IndexViewHolder))
				continue;
			IndexViewHolder holder = (IndexViewHolder) child.getTag();
			final XMLParse parse = holder.getXmlParse();
			if (parse == null)
				continue;
			if (!ParseUtil.mapContainsKey(parse.getMap(),
					FunctionXML.VIDEO_VIEW))
				continue;
			View view = parse.getMap().get(FunctionXML.VIDEO_VIEW);
			if (view instanceof VideoView
					&& !hasFindVideo
					&& ((IndexViewHead) headView).getHelper().getState() != PlayVideoHelper.PLAYING) {
				hasFindVideo = true;
				if (helper.getCurrPlayerItem() == item) {
					...
					helper.stopVideo();
					// 开始播放别的视频
					helper.startVideo(parse.getMap(), item);
				}
				result = item;
			}
		}
		if (!containHeadView) {
			if (headView instanceof IndexViewHead) {
				((IndexViewHead) headView).getHelper().stopVideo();
			}
		}
		return result;
	}
