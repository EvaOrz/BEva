...
public class TagIndexListView implements LoadCallBack, OnScrollListener {
    ...
    private void init() {
        ...
        listView.setonRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh() {
                if (!TextUtils.isEmpty(tagName)) refresh();
            }
    ...
    private void addDefaultHead() {
        if (mContext instanceof ViewsMainActivity) {
            ...
            if (defaultView != null) listView.addHeaderView(defaultView);
        }
    ...
    private void initProperties(String tagName, TagInfoList childList) {
        ...
        if (!TextUtils.isEmpty(template.getListHead().getData())) {
            ...
            View listHead = parse.inflate(template.getListHead().getData(), null, template.getHost());
            listHead.setLayoutParams(new AbsListView.LayoutParams(AbsListView.LayoutParams.FILL_PARENT, AbsListView.LayoutParams.WRAP_CONTENT));
            parse.setDataForListHead(tagInfo);
            listView.addHeaderView(listHead);
        }
        BaseChildCatHead childHead = null;
        // 栏目导航栏
        if (childList != null && ParseUtil.listNotNull(childList.getList()) && template.getCatHead().getCat_list_hold() == 0) {
     ...
     * @param childList
     */
    public void setData(final TagArticleList articleList, final TagInfoList childList, final boolean loadMore, Template temp) {
        template = temp;
        tagName = articleList.getTagName();
        tagInfo = TagInfoListDb.getInstance(mContext).getTagInfoByName(tagName, "", true);
        if (template != null) {
            disPro();
    ...
    private void disPro() {
        if (indexViewPagerItem != null) indexViewPagerItem.disProcess();
     ...
     * @param loadMore
     */
    private void checkShouldInsertSubscribe(TagArticleList dArticleList, boolean loadMore) {
        if (!loadMore && V.checkShouldInsertSubscribeArticle(mContext, tagInfo.getTagName())) {
            // TODO 如果是iweekly新闻栏目，那么获取所有订阅栏目的前5篇文章
            Entry subscribeTopEntry = TagIndexDb.getInstance(mContext).getEntry(new GetTagArticlesOperate(mContext, tagInfo, "", "5", null), "", "", false, TagIndexDb.SUBSCRIBE_TOP_ARTICLE);
            if (subscribeTopEntry instanceof TagArticleList && ParseUtil.listNotNull(((TagArticleList) subscribeTopEntry).getArticleList())) {
                reBuildArticleList(dArticleList, (TagArticleList) subscribeTopEntry, loadMore);
            } else {
        ...
        String s = new Gson().toJson(dArticleList);
        System.out.println();
    }
     ...
     * @param dArticleList
     */
    private void getSubscribeTopArticle(final TagArticleList dArticleList, final boolean loadMore, final TagInfo tagInfo) {
        ...
        if (TextUtils.isEmpty(mergeNames)) {
            ...
            OperateController.getInstance(mContext).getTagIndex(tagInfo, "", "5", null, new FetchEntryListener() {
                ...
                public void setData(Entry entry) {
                    ...
                    if (entry instanceof TagArticleList) {
                        ...
                        reBuildArticleList(dArticleList, (TagArticleList) entry, loadMore);
                    } else {
                        if (indexViewPagerItem != null) {
     ...
     */
    private void reBuildArticleList(TagArticleList dArticleList, TagArticleList sArticleList, boolean loadMore) {
    ...
    private void setDataToIndex(TagArticleList articleList, boolean loadMore) {
        ...
        if (TextUtils.equals(articleList.getViewbygroup(), TagArticleList.BY_CATNAME)) {
            // 按栏目排序没有加载更多
    ...
    private void showHead() {
        if (headView != null) {
            ...
            if (!headView.isShouldScroll() && selfScrollViews.contains(headView)) {
     ...
     * @param articleList
     */
    private void setValueForCatIndex(TagArticleList articleList, boolean loadMore) {
    ...
    public void setSelection(int height) {
        ...
        if (height == -1) height = IndexView.height;
        height = height < 2 ? 0 : height;
        if (currY == height) return;
        currY = height;
    ...
    private void getTagInfo() {
        OperateController.getInstance(mContext).getTagInfo(tagName, FetchApiType.USE_HTTP_ONLY, new FetchEntryListener() {
    ...
    private void checkUpdatetimeIsChanged(TagInfo tagInfo) {
        ...
        String columnUpdateTimeCache = TagDataHelper.getCatIndexUpdateTime(mContext, tagName);
        String articleUpdateTimeCache = TagDataHelper.getCatArticleUpdateTime(mContext, tagName);
        if (TextUtils.equals(columnUpdateTimeCache, columnUpdateTime) && TextUtils.equals(articleUpdateTimeCache, articleUpdateTime)) {
            // NOTE 未改变
            onRefreshed(true);
        } else {
            AppValue.updateTagInfoUpdateTime(tagInfo);
            if (indexViewPagerItem != null && indexViewPagerItem.getTagInfo() != null) {
                indexViewPagerItem.getTagInfo().setColoumnupdatetime(articleUpdateTime);
                indexViewPagerItem.getTagInfo().setArticleupdatetime(articleUpdateTime);
            }
            if (indexViewPagerItem != null) {
    ...
    private void load() {
        if (indexViewPagerItem != null) {
            if (TextUtils.equals(_articleList.getViewbygroup(), TagArticleList.BY_INPUTTIME))
                indexViewPagerItem.loadMore(this, top, _articleList);
            else indexViewPagerItem.loadMore(this, top, null);
        }
    }
    ...
    public void loadMoreArticleList(final TagInfo tagInfo) {
        ...
        OperateController.getInstance(mContext).getTagArticles(mContext, tagInfo, top, "", null, new FetchEntryListener() {
    ...
    private void loadMoreCatIndex(TagInfo catTagInfo) {
        if (catTagInfo == null) return;
        TagArticleList t = TextUtils.equals(_articleList.getViewbygroup(), TagArticleList.BY_INPUTTIME) ? _articleList : null;
        OperateController.getInstance(mContext).getTagIndex(catTagInfo, top, "", t, new FetchEntryListener() {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        if (listView.isHeadShow() || SlateApplication.mConfig.getNav_hide() == 0 || stopCheckNav) {
            return;
        }
        if (mContext instanceof ViewsMainActivity) {
            // TODO 有默认的下拉刷新headview
            if (firstVisibleItem > 1) {
                ((ViewsMainActivity) mContext).callNavPadding(-IndexView.BAR_HEIGHT);
            } else {
                ((ViewsMainActivity) mContext).callNavPadding(getCurrY() - defaultY);
            }
        }
    }

    private int getCurrY() {
        if (defaultView == null) return 0;
        int[] coor = new int[2];
    ...
    public void life(LifeCycle cycle) {
        if (cycle == LifeCycle.PAUSE) {
            ((IndexViewHead) headView).life(cycle);
