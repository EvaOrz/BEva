...
public class IndexViewPagerItem implements Observer, CheckNavHideListener {
	...
	public IndexViewPagerItem(Context context, TagInfo tagInfo,
		...
		if (mContext instanceof ViewsMainActivity) {
			((ViewsMainActivity) mContext).getLifecycleObservable()
					.addObserver(this);
		}
	}
	 ...
	 * @param loadMore
	 */
	private void checkShouldAddMarquee(TagArticleList articleList,
			boolean loadMore) {
		if (loadMore) {
			setValueForIndex(articleList, loadMore);
			return;
	 ...
	 * 获取跑马灯栏目首页
	 */
	private void getMarqueeTagIndex(final TagArticleList articleList,
			final boolean loadMore) {
		OperateController.getInstance(mContext).getTagIndex(
				AppValue.getMarqueeTagInfo(), "", "", null,
				new FetchEntryListener() {
