...
public class XMLDataSetForFav extends BaseXMLDataSet {
    ...
    public XMLDataSetForFav(Context context, HashMap<String, View> map, List<View> clickViewList, List<View> ninePatchViewList, FavAdapter adapter) {
        super(context, map, clickViewList, ninePatchViewList);
        this.adapter = adapter;
    }

    public XMLDataSetForFav(Context context, HashMap<String, View> map, List<View> clickViewList) {
    ...
    public void setData(ArticleItem articleItem, int position) {
        if (map == null || map.isEmpty() || articleItem == null || adapter == null) return;
    ...
    @Override
    protected void onClick(View view, final ArticleItem item, final ArticleType articleType) {
        if (view.getTag(R.id.click) instanceof String) {
            if (TextUtils.equals(view.getTag(R.id.click).toString(), FunctionArticle.BACK)) {
                ((Activity) mContext).finish();
                return;
            }
        }
        LogHelper.logOpenArticleFromFavoriteArticleList(mContext, item.getArticleId() + "", "");
        Intent intent = new Intent(mContext, PushArticleActivity.class);
        intent.putExtra("is_fav",true);
        intent.putExtra(TagProcessManage.KEY_PUSH_ARTICLE, item);
