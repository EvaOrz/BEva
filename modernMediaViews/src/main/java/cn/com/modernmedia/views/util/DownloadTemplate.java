...
public class DownloadTemplate {
    ...
    public DownloadTemplate(Context context, TagArticleList articleList, DownloadTemplateCallBack callBack) {
        mContext = context;
    ...
    public void getTemplate() {
         ...
         */
//        callback(null);
        if (SlateApplication.mConfig.getHas_subscribe() == 0 || AppValue.appInfo.getHaveSubscribe() == 0) {
        ...
        if (TextUtils.isEmpty(name)) {
            ...
            if (!TextUtils.isEmpty(data)) {
                try {
                    new JSONObject(data);
                    callback(ParseProperties.getInstance(mContext).parseIndex(data, getTemplateHost()));
    ...
    private void download() {
        RequestThread thread = new RequestThread(mContext, mUrl, new BaseOperate() {
        ...
        thread.setmFetchDataListener(new FetchDataListener() {
            ...
            public void fetchData(boolean isSuccess, String data, boolean fromHttp) {
                if (isSuccess && !TextUtils.isEmpty(data)) {
                    Template template = ParseProperties.getInstance(mContext).parseIndex(data, getTemplateHost());
                    ...
                    if (!TextUtils.isEmpty(supportVersions)) {
                        ...
                        for (String support : supports) {
                            if (support.equals(SlateApplication.mConfig.getTemplate_version() + "")) {
    ...
    private void fecthLastestTemplate() {
        ...
        if (!TextUtils.isEmpty(lastData)) {
            try {
                ...
                callback(ParseProperties.getInstance(mContext).parseIndex(lastData, getTemplateHost()));
            } catch (JSONException e) {
                e.printStackTrace();
        ...
        if (mCallBack != null) {
            if (template == null) {
                if (mUrl.contains("iweekly-iphone-column-index-gallery")) {
                    template = ParseProperties.getInstance(mContext).parseIndexGalleryNormal();
                } else if (mUrl.contains("iweekly-iphone-column-news-index")) {
                    template = ParseProperties.getInstance(mContext).parseIndexMarqueeNormal();
                } else {
                    template = ParseProperties.getInstance(mContext).parseIndexListNormal();
                }
            }
    ...
    private String getLastestTemplateName() {
        ...
        if (TextUtils.isEmpty(templateName)) return "";
        String s = "\\d+";
    ...
    private String getFolder() {
        return Environment.getExternalStorageDirectory().getPath() + ConstData.DEFAULT_TEMPLATE_PATH;
    }
}
