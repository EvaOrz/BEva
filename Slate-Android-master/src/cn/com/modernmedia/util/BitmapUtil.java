...
public class BitmapUtil {
	...
	private static Options getOptions(String url, String path) {
	...
	public static Bitmap getBitmapByPath(String url, String path, int width,
			int height) {
		Bitmap b = null;
		...
		try {
			...
			Options options = getOptions(url, path);
			if (options != null) {
				int ratio = 1; // 默认为不缩放
				if (width != 0 && height != 0) {
