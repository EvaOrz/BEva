...
import cn.com.modernmedia.model.ArticleItem.ZhuanLanAuthor;
...
public class GetTagArticlesOperate extends BaseIndexAdvOperate {
    ...
    public GetTagArticlesOperate(Context c, TagInfo info, String top, String limited, TagArticleList _articleList) {
        reSetPosition();
        this.tagInfo = info;
        ...
        if (info.getTagLevel() == 2) parent = info.getParent();
        this.top = top;
        isCatIndex = false;
        this.limited = limited;
        this.url = UrlMaker.getArticlesByTag(c,info, top, limited);
        if (_articleList == null) articleList = new TagArticleList();
        else articleList = _articleList;
        cacheIsDb = true;
    ...
    protected String getUrl() {
        ...
        Log.e("GetTagArticlesOperate", url);
        return url;
    }
    ...
    protected void handler(JSONObject jsonObject) {//接口文章列表数据
        ...
        if (TextUtils.equals(limited, "5")) {
            if (isCatIndex) {
                ...
                TagIndexDb.getInstance(getmContext()).saveSubscribeTopArticle(articleList);
            } else {
                TagArticleListDb.getInstance(getmContext()).clearSubscribeTopArticle();
                TagArticleListDb.getInstance(getmContext()).saveSubscribeTopArticle(articleList);
            }
            return;
        }
        if (TextUtils.isEmpty(top)) {
    ...
    protected void parseArr(JSONArray arr) {
        ...
        articleList.setProperty(parseColumnProperty(jsonObject.optJSONObject("phoneColumnProperty")));
        articleList.setViewbygroup(jsonObject.optString("viewbygroup"));
        articleList.setIsRadio(jsonObject.optInt("isRadio"));
        articleList.setShowCount(jsonObject.optInt("showCount"));
        articleList.setType(jsonObject.optInt("type"));
        articleList.setLink(jsonObject.optString("link"));
        articleList.setEnablesubscribe(jsonObject.optInt("enablesubscribe"));
    ...
    public void parseArticleItem(JSONObject obj) {
        ArticleItem item = parseSubArticle(obj);
        setSort(item);
    ...
    public ArticleItem parseSubArticle(JSONObject obj) {
        ...
        item.setSubcat(obj.optString("subcat"));
        if (tagInfo != null) {
        ...
        // 推荐首页title color
        item.setGroupdisplaycolor(transformColor(obj.optString("groupdisplaycolor")));
        // 解析专栏栏目聚合作者信息
        item.setZhuanlanAuthor(parseZhuanLanAuthor(obj.optJSONObject("aritclegroup")));
        // 文章url地址解析
        ...
        if (!isNull(subListArr)) {
            item.setSubArticleList(parseSubList(subListArr));
        }
    ...
    private List<ArticleItem> parseSubList(JSONArray subListArr) {
        List<ArticleItem> subList = new ArrayList<ArticleItem>();
        try {
            for (int j = 0; j < subListArr.length(); j++) {
                subList.add(parseSubArticle(subListArr.getJSONObject(j)));
            }
        } catch (JSONException e) {
    ...
    private void setSort(ArticleItem item) {
        ...
        // todo
        if (ConstData.getAppId() == 37 && item.getPosition().getId() == 2) {
            String date = format.format(new Date(item.getInputtime() * 1000L));
        ...
        if (TextUtils.isEmpty(articleList.getViewbygroup()) || item.getPosition().getId() == 1)
            return;
        /**
        ...
        if (TextUtils.equals(articleList.getViewbygroup(), TagArticleList.BY_CATNAME)) {
            // 根据栏目排序
            if (item.getPosition().getId() != 3 || TextUtils.isEmpty(item.getGroupname())) return;
            if (!groupList.contains(item.getGroupname())) {// 即时头条不显示栏目条
                if (!item.getGroupname().equals("cat_32")) item.setShowTitleBar(true);
                groupList.add(item.getGroupname());
                currentSection++;
                currentPositionInSection = 0;
            }
        } else if (TextUtils.equals(articleList.getViewbygroup(), TagArticleList.BY_INPUTTIME)) {
            String date = format.format(new Date(item.getInputtime() * 1000L));
            if (!articleList.getDateList().contains(date)) {
                if (!item.getGroupname().equals("cat_32")) item.setShowTitleBar(true);
                articleList.getDateList().add(date);
            }
        } else if (TextUtils.equals(articleList.getViewbygroup(), TagArticleList.BY_AUTHOR)) {
            // 按作者排序
            if (item.getPosition().getId() != 3 || TextUtils.isEmpty(item.getGroupname())) return;
    ...
    private IndexProperty parseProperty(JSONObject obj) {
        ...
        if (!isNull(obj)) {
            ...
            property.setIsadv(obj.optInt("isadv",0));
        }
        return property;
    }
     ...
     * @return
     */
    private ZhuanLanAuthor parseZhuanLanAuthor(JSONObject obj) {
        ZhuanLanAuthor zhuanLanAuthor = new ZhuanLanAuthor();
        if (!isNull(obj)) {
            zhuanLanAuthor.setId(obj.optString("id"));
            zhuanLanAuthor.setName(obj.optString("name"));
            zhuanLanAuthor.setColor(transformColor(obj.optString("color")));
            zhuanLanAuthor.setDesc(obj.optString("desc"));
            zhuanLanAuthor.setPicture(obj.optString("picture"));
    ...
    protected CallBackData fetchDataFromDB() {
        ...
        Entry entry;
        if (isCatIndex) {
            entry = TagIndexDb.getInstance(getmContext()).getEntry(this, tagName, top, true, "");
        } else {
            entry = TagArticleListDb.getInstance(getmContext()).getEntry(this, tagName, top, false, "");
        }
        if (entry instanceof TagArticleList) {
            ...
            if (hasData) {
                addLarger();
                PrintHelper.print("from db:getTagArticleList=" + tagName + "====" + url);
