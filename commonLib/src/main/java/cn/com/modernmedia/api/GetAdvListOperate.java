...
public class GetAdvListOperate extends BaseOperate {
    ...
    protected void handler(JSONObject jsonObject) {
        ...
        if (!isNull(adver)) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                advItem.setAutoClose(obj.optInt("autoClose", 0));
                ...
                if (advItem.getAdvType() == AdvList.BETWEEN_ARTICLE && TextUtils.isEmpty(advItem.getTagname())) {
                ...
                advItem.setName(obj.optString("name"));
                advItem.setTargetTag(obj.optString("targetTag"));
                ...
                if (!advList.getAdvMap().containsKey(advItem.getAdvType())) {
                    advList.getAdvMap().put(advItem.getAdvType(), new ArrayList<AdvItem>());
                }
    ...
    private List<String> parseAdvLinks(JSONArray arr) {
        ...
        if (isNull(arr)) return links;
        int length = arr.length();
        JSONObject obj;
        for (int i = 0; i < length; i++) {
            obj = arr.optJSONObject(i);
            if (!isNull(obj)) links.add(obj.optString("url"));
        }
        return links;
    }
    ...
    private void parseTrackerUrl(JSONObject obj, AdvItem item) {
        if (!isNull(obj)) {
            item.getTracker().setImpressionUrl(obj.optString("impressionUrl", ""));
