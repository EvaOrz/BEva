...
public class GetTagInfoOperate extends BaseOperate {
    ...
    public GetTagInfoOperate(String parentTagName, String tagName, String group, String top, TAG_TYPE type) {
        ...
        if (type == TAG_TYPE.APP_INFO) {
            ...
            url = UrlMaker.getTagChild("", tagName, group, top, true, true);
        } else if (type == TAG_TYPE.CHILD_CAT) {
            url = UrlMaker.getTagChild(parentTagName, "", group, top, true, false);
        } else if (type == TAG_TYPE.ALL_CAT) {
            url = UrlMaker.getBookColumns();
        } else if (type == TAG_TYPE.SPECIAL) {
            url = UrlMaker.getSpecial(top);
        } else if (type == TAG_TYPE.EBOOK) {
            url = UrlMaker.getEbookTag(top);
        } else if (type == TAG_TYPE.FM) {
            url = UrlMaker.getFmTag();
        } else if (type == TAG_TYPE.SEARCH_HOTS) {
            url = UrlMaker.getSearchTags(top);
        }
    ...
    protected void handler(JSONObject jsonObject) {
        ...
        TagInfoListDb.getInstance(getmContext()).clearTable(parentTagName, tagName, group, type);
    ...
    protected CallBackData fetchDataFromDB() {
        ...
        Entry entry = TagInfoListDb.getInstance(getmContext()).getEntry(this, parentTagName, tagName, group, top, type);
        if (entry instanceof TagInfoList) {
            ...
            if (ParseUtil.listNotNull(list.getList())) {
                ...
                if (type == TAG_TYPE.APP_INFO)
                    Log.e("from db:", "====" + list.getList().get(0).getAppProperty().getUpdatetime());
            }
        }
        return callBackData;
    }
        ...
        if (!isNull(array)) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (!isNull(object)) {
                    ...
                    tagInfo.setDefaultsubscribe(object.optInt("defaultsubscribe"));
                    tagInfo.setIsFix(object.optInt("isfix"));
                    tagInfo.setTagLevel(object.optInt("tagLevel"));
                    tagInfo.setArticleupdatetime(object.optString("articleupdatetime"));
                    tagInfo.setColoumnupdatetime(object.optString("coloumnupdatetime"));
                    tagInfo.setPublishTime(object.optString("publishtime"));
                    tagInfo.setAppProperty(parseAppProperty(object.optJSONObject("appProperty")));
                    tagInfo.getAppProperty().setUpdatetime(object.optString("updatetime"));
                    tagInfo.getAppProperty().setAdvUpdateTime(object.optString("advUpdateTime"));
                    tagInfo.setIssueProperty(parseIssueProperty(object.optJSONObject("issueProperty")));
                    tagInfo.setColumnProperty(parseColumnProperty(object.optJSONObject("phoneColumnProperty"), tagInfo.getTagName()));
                    tagInfo.setIsRadio(object.optInt("isRadio"));
                    tagInfo.setType(object.optInt("type"));
                    tagInfo.setIsPay(object.optInt("ispay"));
                    tagInfo.setCatName(object.optString("catname"));
                    tagInfo.setDesc(object.optString("desc"));
                    tagInfo.setOffset(object.optString("offset"));
    ...
    public AppProperty parseAppProperty(JSONObject object) {
        ...
        if (!isNull(object)) {
            ...
            property.getVersion().setDownload_url(object.optString("download_url"));
            property.setSplash(object.optString("splash"));
            property.setBottomBarPkg(object.optString("bottomBarPkg"));
    ...
    public ColumnProperty parseColumnProperty(JSONObject object, String tagName) {
        ...
        if (!isNull(object)) {
            ...
            parseSubscriptPicture(property, object.optJSONArray("subscriptPicture"));
            ...
            if (ParseUtil.listNotNull(property.getSubscriptPicture())) {
                DataHelper.columnPicMap.put(tagName, property.getSubscriptPicture());
            }
    ...
    public IssueProperty parseIssueProperty(JSONObject object) {
        ...
        if (!isNull(object)) {
            ...
            property.setStartTime(object.optLong("starttime"));
            property.setEndTime(object.optLong("endtime"));
            ...
            if (!isNull(array)) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (!isNull(picObject)) {
                        property.getPictureList().add(picObject.optString("url"));
                    }
                }
            }
        }
