...
public class OperateController {
	 ...
	 * @param listener
	 */
	public void saveSubscribeColumnSingle(String uid, String token,
			SubscribeColumn data, FetchEntryListener listener) {
		SaveUserSubcribeSingleOperate operate = new SaveUserSubcribeSingleOperate(
				uid, token, data);
		doPostRequest(operate, operate.getError(), FetchApiType.USE_HTTP_ONLY,
				listener);
	}
	 ...
	 * 添加日志(参数方式)
	 */
	public void addSelection(Context context, JSONObject array,
			FetchEntryListener listener) {
	 ...
	 * 添加日志（文件方式）
	 */
	public void addSelectionByFile(Context context, String path,
			FetchEntryListener listener) {
	...
	/**
	 * 获取用户支付权限 1. 启动app，若登陆状态，取支付权限 2. 登陆成功， 3. 第三方登陆成功
	 */
	public void getUserPermission(Context context, String token,
			FetchEntryListener listener) {
		GetUserPermissionOperate operate = new GetUserPermissionOperate(
				context, token);
		doPostRequest(operate, operate.getPermission(),
				FetchApiType.USE_HTTP_ONLY, listener);
	}
	 ...
	 * @param listener
	 */
	public void notifyServerPayResult(Context context,String aliOrWeixin,String data,
			FetchEntryListener listener) {
		PayResultOperate operate = new PayResultOperate(context, aliOrWeixin, data);
		doPostRequest(operate, operate.getError(), FetchApiType.USE_HTTP_ONLY,
				listener);
	}
	...
	// * @param listener
	// */
	// public void notifyServerWeixinPayResult(Context context, String
	// jsonString,
	// FetchEntryListener listener) {
	// PayResultOperate operate = new PayResultOperate(context, "weixin",
	// jsonString);
	// doPostRequest(operate, operate.getError(), FetchApiType.USE_HTTP_ONLY,
	// listener);
	// }
	 ...
	 * 获取支付商品列表
	 */
	public void getProducts(FetchEntryListener listener) {
		GetProductsOperate operate = new GetProductsOperate();
		doRequest(operate, operate.getPros(), FetchApiType.USE_HTTP_ONLY,
				listener);
	}
}
