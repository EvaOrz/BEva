...
public class NewFavDb extends SQLiteOpenHelper {
    ...
    private String getCreatSql() {
        ...
        helper.addColumn(LEVEL, "INTEGER");//16
    ...
    private List<ArticleItem> getLocalFav(String uid, String where) {
        ...
        Cursor cursor = null;
        try {
            cursor = db.query(TABLE_NAME, null, where, null, null, null, DATE + " desc");
            while (cursor != null && cursor.moveToNext()) {
                ...
                int type = ParseUtil.stoi(cursor.getString(cursor.getColumnIndex(TYPE)), 1);
                detail.getProperty().setType(type);
                detail.getProperty().setLevel(cursor.getInt(cursor.getColumnIndex(LEVEL)));

                detail.setSlateLink(cursor.getString(cursor.getColumnIndex(LINK)));
                Picture picture = new Picture();
                picture.setUrl(cursor.getString(cursor.getColumnIndex(PICTURE)));
                detail.getThumbList().add(picture);
                detail.setUpdateTime(cursor.getLong(cursor.getColumnIndex(UPDATETIME)));
                ...
                if (type == 2) { // 图集数据从图集表中取得
                    detail.getPageUrlList().addAll(atlasTable.getAtlas(db, detail.getArticleId(), uid));
                } else {
                    PhonePageList phonePage = new PhonePageList();
                    phonePage.setUrl(cursor.getString(cursor.getColumnIndex(PAGE)));
    ...
    public void addFav(ArticleItem favorite, String uid, boolean success) {
        ...
        try {
            ...
            db.insert(TABLE_NAME, null, createContentValues(favorite, uid, success, false));
            if (favorite.getProperty().getType() == 2) { // 图集
                atlasTable.insert(db, favorite.getArticleId(), uid, favorite.getPageUrlList());
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
     ...
     * @param uid
     * @param success
     * @param isDataTransfer 是否数据迁移
     */
    private void addFavs(List<ArticleItem> list, String uid, boolean success, boolean isDataTransfer) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.beginTransaction();
        try {
            for (ArticleItem item : list) {
                db.delete(TABLE_NAME, getWhere(item.getArticleId(), uid), null);
                db.insert(TABLE_NAME, null, createContentValues(item, uid, success, isDataTransfer));
    ...
    private ContentValues createContentValues(ArticleItem favorite, String uid, boolean success, boolean isDataTransfer) {
        ContentValues cv = new ContentValues();
        cv.put(ID, favorite.getArticleId());
        cv.put(TITLE, favorite.getTitle());
        cv.put(LINK, favorite.getSlateLink()); // 文章link
        if (ParseUtil.listNotNull(favorite.getPageUrlList()) && favorite.getPageUrlList().get(0) != null) {
            PhonePageList phonePage = favorite.getPageUrlList().get(0);
            cv.put(PAGE, phonePage.getUrl());
        }
        // 没有缩略图时用大图替换
        if (ParseUtil.listNotNull(favorite.getThumbList()) && favorite.getThumbList().get(0) != null && !TextUtils.isEmpty(favorite.getThumbList().get(0).getUrl())) {
            cv.put(PICTURE, favorite.getThumbList().get(0).getUrl());
        } else if (ParseUtil.listNotNull(favorite.getPicList()) && !TextUtils.isEmpty(favorite.getPicList().get(0).getUrl())) {
            cv.put(PICTURE, favorite.getPicList().get(0).getUrl());
        }
        cv.put(UPDATETIME, favorite.getUpdateTime());
        cv.put(TYPE, favorite.getProperty().getType());
        cv.put(LEVEL, favorite.getProperty().getLevel());
        if (isDataTransfer) {
        ...
        if (isDataTransfer) cv.put(UID, favorite.getDbData().getUid());
        else cv.put(UID, uid);
        cv.put(APPID, ConstData.getInitialAppId() + "");
        cv.put(DESC, favorite.getDesc());
        cv.put(FAVDEL, favorite.getFavDel() == 0 ? 0 : 1);
        if (isDataTransfer) cv.put(SUCCESS, favorite.getDbData().getSuccess());
        else cv.put(SUCCESS, success ? 1 : 0);
    ...
    public boolean containThisFav(int id, String uid) {
        ...
        Cursor c = null;
        try {
            c = db.query(TABLE_NAME, columns, ID + "=" + id + " AND " + UID + "=" + uid + " AND " + FAVDEL + "=0", null, null, null, null);
            return c.moveToNext();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    private String getWhere(int articleId, String uid) {
        String where = "";
        if (articleId != -1) where = ID + " = " + articleId;
        ...
        if (TextUtils.isEmpty(uid) || uid.equals(SlateApplication.UN_UPLOAD_UID)) {
            ...
            where += and + UID + " in(" + SlateApplication.UN_UPLOAD_UID + "," + uid + ")";
        }
        return where;
    }
    ...
    public static class AtlasTable {
        ...
        private ContentValues createContentValues(int articleId, String uid, PhonePageList page) {
            ContentValues cv = new ContentValues();
         ...
         * @param page
         */
        public void insert(SQLiteDatabase db, int articleId, String uid, PhonePageList page) {
            db.insert(TABLE_NAME, null, createContentValues(articleId, uid, page));
        }
         ...
         * @param pageList
         */
        public void insert(SQLiteDatabase db, int articleId, String uid, List<PhonePageList> pageList) {
         ...
         * @return
         */
        public List<PhonePageList> getAtlas(SQLiteDatabase db, int articleId, String uid) {
            List<PhonePageList> list = new ArrayList<PhonePageList>();
            Cursor cursor = null;
            try {
                cursor = db.query(TABLE_NAME, null, getWhere(articleId, uid), null, null, null, null);
                while (cursor != null && cursor.moveToNext()) {
                    ...
                    page.setUrl(cursor.getString(cursor.getColumnIndex(IMAGE_URL)));
