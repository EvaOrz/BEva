...
public abstract class CommonArticleActivity extends BaseActivity {
    ...
    public BindFavToUserListener bindFavToUserListener;
     ...
     * 设置页面
     *
     * @param layoutResID 页面layout_id,可传-1，代表使用默认页面
     */
    @Override
    public void setContentView(int layoutResID) {
        ...
        if (transferArticle == null) return;
        init();
        initProcess();
        initViewpager();
        fetchData(true);
    ...
    private void fetchData(boolean delay) {
        if (transferArticle.getArticleType() == ArticleType.Fav) { // 收藏
            ...
            //            LastIssueAticleActivityHelper lastHelper = new LastIssueAticleActivityHelper(this, transferArticle);
    ...
    private void getArticleList() {
        ...
        if (TextUtils.isEmpty(transferArticle.getParent())) {
            tagInfo = TagInfoListDb.getInstance(this).getTagInfoByName(transferArticle.getTagName(), "", true);
        } else {
            tagInfo = TagInfoListDb.getInstance(this).getTagInfoByName(transferArticle.getTagName(), transferArticle.getParent(), false);
        }
    ...
    private void getTagArticleList(TagInfo tagInfo, boolean useCache) {
        ...
        if (useCache) {
            OperateController.getInstance(this).getTagArticles(this, tagInfo, "", "", null, FetchApiType.USE_CACHE_FIRST, new FetchEntryListener() {
            ...
            OperateController.getInstance(this).getTagArticles(this, tagInfo, "", "", null, new FetchEntryListener() {
    ...
    private void getTagInfo() {
        OperateController.getInstance(this).getTagInfo(transferArticle.getTagName(), FetchApiType.USE_CACHE_FIRST, new FetchEntryListener() {
    ...
    private void getArticle(int articleId, final boolean resetAdapter) {
        showLoading();
        OperateController.getInstance(this).getArticleDetails(articleId, new FetchEntryListener() {
            ...
            public void setData(Entry entry) {
                if (entry instanceof TagArticleList) {
                    ...
                    if (ParseUtil.listNotNull(tagArticleList.getArticleList())) {
                        // 把这个文章添在列表末尾
                        ArticleItem item = tagArticleList.getArticleList().get(0);
                        list.addAll(checkMultiplePage(item));
                        final int pos = list.size() - item.getPageUrlList().size();
                        if (resetAdapter) {
            ...
            public void updateDes(int position) {
                ...
                if (needDeleteHidden != -1) {
                    if (Math.abs(position - needDeleteHidden) > 1) {
                        ...
                        adapter.destroyItem(viewPager, position, viewPager.findViewWithTag(1));
                        needDeleteHidden = -1;
                ...
                //                hideKeyButton(true, type == 2, item.isAdv() != 0, item.isAdv() != 0);
                changeFav(position);
                ...
                if (loadOkIds.contains(currArticleId)) {
                    ...
                    LogHelper.logAndroidShowArticle(mContext, transferArticle.getTagName(), item.getArticleId() + "");
                }
            }
        });
    }
    ...
    private void initDataFromBundle() {
        if (getIntent() != null && getIntent().getExtras() != null) {
            transferArticle = (TransferArticle) getIntent().getExtras().get(GenericConstant.TRANSFE_RARTICLE);
        }
    }
    ...
    private List<ArticleItem> checkMultiplePage(ArticleItem detail) {
        ...
        if (detail.getPageUrlList().size() > 1 && detail.getProperty().getType() == 1) {
            // pagelist多页
    ...
    protected void getPosition() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // 去掉滑动时需要隐藏的文章
            if (detail.getProperty().getScrollHidden() == 1 && detail.getArticleId() != transferArticle.getArtcleId())
                continue;
        ...
        int pos = -1;
        if (transferArticle.getArtcleId() == -1 && transferArticle.getAdvId() == -1) {
            pos = 0;
        } else {
            length = list.size();
            for (int i = 0; i < length; i++) {
                ...
                } else if (list.get(i).getArticleId() == transferArticle.getArtcleId()) {
                    pos = i;
                    break;
                }
            }
    ...
    protected void changeFav(int pos) {
        ...
        if (detail != null) {
            if (db.containThisFav(detail.getArticleId(), transferArticle.getUid())) {
                changeFavBtn(true);
            } else {
                changeFavBtn(false);
            }
     ...
     */
    public void addFav() {
        if (!ParseUtil.listNotNull(list)) return;
        ...
        if (list.size() > position) {
            ...
            // 判断用户付费身份
            if (fav.getProperty().getLevel() > 0 && !SlateDataHelper.getLevelByType(mContext, fav.getProperty().getLevel())) {
                Toast.makeText(mContext, R.string.pay_for_fav, Toast.LENGTH_SHORT).show();
            } else {// 请付费后收藏
    ...
    private void changeFont() {
        if (!listenerMap.isEmpty()) for (int position : listenerMap.keySet()) {
    ...
    private void loadViewAfterFont() {
        ...
        LogHelper.logChangeArticleFontSize(this, detail.getArticleId() + "", transferArticle.getTagName());
    ...
    private class ViewPageAdapter extends MyPagerAdapter<ArticleItem> {
        ...
        public void destroyItem(ViewGroup container, int position, Object object) {
            ...
            if (listenerMap.containsKey(position) && position != currentPosition) {
        ...
        private void changePage(View view, boolean push) {
            if (!(view instanceof ArticleDetailItem)) return;
            ArticleDetailItem detailView = (ArticleDetailItem) view;
            if (detailView == null || detailView.getDetail() == null || detailView.getWebView() == null || !loadOkIds.contains(detailView.getDetail().getArticleId())) {
                return;
            }
            if (push) {
     ...
     * @param needComment
     */
    protected abstract void hideKeyButton(boolean needFav, boolean needFont, boolean needShare, boolean needComment);
    ...
    //     * @param hide
    //     */
    //    protected abstract void hideIfFav(boolean hide);
    ...
    protected void onResume() {
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (!listenerMap.isEmpty()) for (int position : listenerMap.keySet()) {
                    listenerMap.get(position).checkPayStatus();
                }
            }
        }, 500);
    ...
    protected void onPause() {
        super.onPause();
        //        if (currView == null || !(currView instanceof ArticleDetailItem)) return;
        //        ArticleDetailItem detailView = (ArticleDetailItem) currView;
        //        ((ArticleDetailItem) currView).getWebView().reload();
    }
