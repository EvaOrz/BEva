...
public class AdvList extends Entry {
     ...
     * 商城焦点位广告
     */
    public static final Integer SHANG_HEAD = 11;
    /**
     * 商城焦点位下方广告
     */
    public static final Integer SHANG_UNDERHEAD = 12;

    /**
     * 我的vip页面广告
     */
    public static final Integer MYVIP_ADV = 13;
    /**
     * 站内信广告
     */
    public static final Integer ZHANNEI_ADV = 14;
    ...
    public static class AdvItem extends Entry {
        ...
        private int showType;// 0为图片(当为文章建广告时，为组图)，1为网页,2为pdf,3.以文章形式展现的广告,4.带链接的视频
        ...
        private String targetTag = "";// 站内信标识
        ...
        public TagInfoList.TagInfo convertToTagInfo() {
            TagInfoList.TagInfo tagInfo = new TagInfoList.TagInfo();
            tagInfo.setAdvItem(this);
            TagInfoList.ColumnProperty c = new TagInfoList.ColumnProperty();
            c.setCname(getName());
            tagInfo.setColumnProperty(c);
            tagInfo.setGroup(-1);
            return tagInfo;
        }
        ...
        public ArticleItem convertToArticleItem() {
            ...
            if (showType == 0) item.setAdv(1);
            else if (showType == 1) item.setAdv(2);
            else if (showType == 4) item.setAdv(4);
            if (ParseUtil.listNotNull(sourceList)) {
                ...
                picture.setUrl(source.getUrl());
                picture.setVideolink(source.getVideolink());
                item.getPicList().add(picture);
                item.setSlateLink(source.getLink());
                item.setWeburl(source.getWeburl());
                item.setSlateLinkList(source.getLinks());
                item.setAdvSource(source);
                item.getPosition().setStyle(101);
                item.setAppid(appId);
            }
            if (posInfo != null) {
