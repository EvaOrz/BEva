...
public class TagArticleList extends Entry {
    ...
    public static final String BY_AUTHOR = "byauthor";// 根据作者排序（专栏）
    ...
    private String link = "";
    // 是否可以订阅
    private int enablesubscribe = 0;// 0：不可订阅，1：可订阅
    ...
    private int showCount = 0;// 0：不显示fm播放次数 1：显示播放次数
    ...
    public TagArticleList copy() {
        ...
        tagArticleList.setEnablesubscribe(enablesubscribe);
        ...
        for (int key : map.keySet()) {
            ...
            for (ArticleItem item : list) {
                if (!tagArticleList.getMap().containsKey(key)) {
                    tagArticleList.getMap().put(key, new ArrayList<ArticleItem>());
                }
     ...
     * @param sArticleList
     */
    public void insertSubscribeArticle(Context context, TagArticleList sArticleList, boolean isIndex) {
        if (sArticleList == null || !ParseUtil.listNotNull(sArticleList.getArticleList())) {
            return;
        }
        sArticleList.getArticleList().get(0).setShowTitleBar(true);
//        sArticleList.getArticleList().get(0).setInputtime(context.getString(R.string.theme_magazine));
        if (dateList.size() <= 1) {
    ...
    public boolean hasData(int position) {
        return map.containsKey(position) && ParseUtil.listNotNull(map.get(position));
    }
