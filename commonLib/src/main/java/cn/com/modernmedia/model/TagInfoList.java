...
public class TagInfoList extends Entry {
    ...
    public boolean hasChild(String parent) {
        return ParseUtil.listNotNull(getChildHasSubscriptTagInfoList(parent).getList());
    }
    ...
    public TagInfoList getEnSubscriptMap(String parent) {
        TagInfoList tagInfoList = new TagInfoList();
        if (childMap.isEmpty()) return tagInfoList;
     ...
     * 获取栏目列表显示的
     *
     * @param isSubscript 是否获取订阅栏目
     * @param addPic      添加一个显示"订阅"图片的空栏目
     * @return
     */
    public List<TagInfo> getColumnTagList(boolean isSubscript, boolean addPic) {
        List<TagInfo> result = new ArrayList<TagInfo>();
        for (TagInfo tagInfo : list) {
            if (tagInfo.getTagLevel() != 1 || tagInfo.getHasSubscribe() == 0) continue;
            if (isSubscript) {
    ...
    public String getSubscriptTagMergeName() {
        ...
        for (TagInfo info : list) {
            if (info.getHasSubscribe() == 0 || info.getTagLevel() != 1 || info.getAppId() == ConstData.getInitialAppId())
                continue;
    ...
    public TagInfoList copy() {
        ...
        if (childMap.keySet().isEmpty()) return result;
        for (String key : childMap.keySet()) {
            List<TagInfo> list = childMap.get(key);
            for (TagInfo tagInfo : list) {
    ...
    public boolean checkStatusIsSame(TagInfoList o) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (info.getTagLevel() != 1) continue;
            if (info.getHaveChildren() == 1) {
                // 父栏目，那么判断子栏目订阅状态是否一致
                if (!checkChildStatusIsSame(o, info.getTagName())) return false;
            } else {
                // 只有不是父栏目的一级栏目才判断isFix
                if (info.getIsFix() == 1) continue;
                if (info.getColumnProperty().getNoColumn() == 1) continue;
                // 普通一级栏目，直接判断订阅状态是否一致
                if (currList.size() > i) {
                    TagInfo curr = currList.get(i);
                    if (curr.getHasSubscribe() != info.getHasSubscribe()) return false;
                } else {
                    return false;
                }
            }
    ...
    private boolean checkChildStatusIsSame(TagInfoList o, String parentName) {
        ...
        if (!ParseUtil.mapContainsKey(getChildMap(), parentName)) return false;
    ...
    public static class TagInfo extends Entry {
        ...
        // 1.app类 2.issue类 3.column类 4.独立栏目 12.特刊 -1:广告类（网页、图片）
        private int group = 0;
        ...
        private AdvList.AdvItem advItem = null;// 广告
        ...
        private String catName = "";// 订阅页面显示tagname
        private String desc = "";// 专题摘要
        private String offset = "";// 分页标签
        ...
        public AdvList.AdvItem getAdvItem() {
            return advItem;
        }

        public void setAdvItem(AdvList.AdvItem advItem) {
        ...
        public TagInfo(String name) {
            this.columnProperty.setCname(name);
        }

        public TagInfo(int appId, String tagName, String parent, int haveChildren, String link, String articleupdatetime, String coloumnupdatetime, int group, int enablesubscribe, int defaultsubscribe, int isFix, int hasSubscribe, int tagLevel, String publishTime, AppProperty appProperty, IssueProperty issueProperty, ColumnProperty columnProperty, int isRadio, int type, String desc, String offset) {
            super();
            this.appId = appId;
        ...
        public TagInfo copy() {
            return new TagInfo(appId, tagName, parent, haveChildren, link, articleupdatetime, coloumnupdatetime, group, enablesubscribe, defaultsubscribe, isFix, hasSubscribe, tagLevel, publishTime, appProperty, issueProperty, columnProperty, isRadio, type, desc, offset);
        }
        ...
        public boolean showChildren() {
            return haveChildren == 1 && AppValue.ensubscriptColumnList.hasChild(tagName) && columnProperty.getIsMergeChild() == 0;
        }

        /**
         * 获取父栏目下所有子栏目的merge
         *
         * @param checkMerge 是否只取isMergeChild=1的父栏目
         * @return 如果不为空，代表需要
         */
        public String getMergeName(boolean checkMerge) {
            String mergeTagName = "";
            if (haveChildren == 0) return "";
            if (checkMerge && columnProperty.getIsMergeChild() == 0) return "";
            TagInfoList childInfoList = AppValue.ensubscriptColumnList.getChildHasSubscriptTagInfoList(tagName);
            for (TagInfo child : childInfoList.getList()) {
                mergeTagName += child.getTagName() + ",";
            }
            if (mergeTagName.endsWith(","))
                mergeTagName = mergeTagName.substring(0, mergeTagName.length() - 1);
            return mergeTagName;
        }
        ...
        public TagInfo getMergeParentTagInfo(Context context) {
            if (tagLevel == 2) {
                TagInfo parentInfo = TagInfoListDb.getInstance(context).getTagInfoByName(parent, "", true);
    ...
    public static class AppProperty extends Entry {
        ...
        private String bottomBarPkg = "";// 商业周刊首页底部菜单zip包

        public String getBottomBarPkg() {
            return bottomBarPkg;
        }

        public void setBottomBarPkg(String bottomBarPkg) {
            this.bottomBarPkg = bottomBarPkg;
        }
