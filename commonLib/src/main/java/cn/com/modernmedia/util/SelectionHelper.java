...
public class SelectionHelper {
    ...
    private static String path = FileManager.getDefaultPath() + ConstData.DEFAULT_DATA_PATH;

    public static synchronized SelectionHelper getInstance() {
        if (instance == null) instance = new SelectionHelper();
        return instance;
    }
    ...
    public void add(Context context, String event, Map<String, String> map) {
        ...
        try {
            ...
            addPostParams(postObject, "timestamp", System.currentTimeMillis() / 1000 + "");

            arrayevent.put(postObject);// 添加
            if (arrayevent.length() > 9) {// post参数长度限制，50条请求一次接口
    ...
    public void sendApi(Context context, final JSONArray array) {
        ...
        try {
            ...
            addPostParams(json, "deviceid", Tools.getDeviceId(context));
            addPostParams(json, "devicemodel", android.os.Build.MODEL);// 手机型号
            addPostParams(json, "appid", ConstData.getInitialAppId() + "");

            addPostParams(json, "appversion", Tools.getAppVersionName(context));

            addPostParams(json, "os", "android");
            addPostParams(json, "osversion", android.os.Build.VERSION.RELEASE + android.os.Build.VERSION.SDK);
            addPostParams(json, "appbuildversion", Tools.getAppVersionCode(context));
            addPostParams(json, "appname", context.getResources().getString(R.string.app_name));
        } catch (Exception e) {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                        ...
                        Log.e("rizhi shangchuan", resData);
    ...
    public void sendApiFile(Context context) {
        try {
            ...
            OperateController controller = OperateController.getInstance(context);
            controller.addSelectionByFile(context, path + "events_log_zip.zip", new FetchEntryListener() {
     ...
     * @throws Exception
     */
    private void addPostParams(JSONObject obj, String key, String value) throws Exception {
