...
public class EnsubscriptHelper {
    ...
    public static void addEnsubscriptColumn(Context context) {
        ...
        for (TagInfo t : AppValue.ensubscriptColumnList.getList()) {// 循环全部订阅列表
            ...
            if (t.getIsFix() == 1)// 固定订阅--》加入用户订阅临时列表
                ...
                for (int i = 0; i < subscribeTagList.size(); i++) {//
                    if (subscribeTagList.get(i).equals(t.getTagName())) {// 已订阅--》加入用户订阅临时列表
                        AppValue.bookColumnList.getList().add(t);
                    }
                }
            }
    ...
    // * 用户已订阅列表
    // */
    // private static void hasSubscribeTagListByUser(List<String>
    // subscribeTagList) {
    // for (TagInfo tagInfo : AppValue.ensubscriptColumnList.getList()) {
