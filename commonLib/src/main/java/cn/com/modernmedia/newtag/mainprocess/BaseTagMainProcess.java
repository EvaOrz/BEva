...
public abstract class BaseTagMainProcess {
	...
	protected void checkAfterFetchAdvList(FetchApiType apiType) {
		if (ConstData.getAppId() == 20) {
			...
			new FetchShiyeIndexHelper(mContext) {
				...
				@Override
				public void doAfterFecthShiye(TagArticleList articleList,
						boolean success) {
					BaseTagMainProcess.this.doAfterFecthShiye(articleList,
							success);
				}
	...
	protected void getCatList(final FetchApiType type) {
		...
		TimeCollectUtil.getInstance().addCollectUrl(
				UrlMaker.getCatList((AppValue.appInfo.getTagName())));
		mController.getTagInfo(AppValue.appInfo.getTagName(), "", "3", "",
				TAG_TYPE.TREE_CAT, type, new FetchEntryListener() {
	...
	protected void getLastestArticle(final FetchApiType type) {
		TimeCollectUtil.getInstance().addCollectUrl(
				UrlMaker.getWeeklyLatestArticleId((AppValue.appInfo
						.getTagName())));
		OperateController.getInstance(mContext).getLatestArticleIds("", type,
	...
	protected void getSubscribeList(FetchApiType type) {
		if (TextUtils.equals(Tools.getUid(mContext),
	...
	protected void getSubscribeOrderList(FetchApiType type) {
		errorType = 5;
		TimeCollectUtil.getInstance().addCollectUrl(
				UrlMaker.getUserSubscribeList(Tools.getUid(mContext),
						SlateDataHelper.getToken(mContext)));
		mController.getSubscribeOrderList(Tools.getUid(mContext),
				SlateDataHelper.getToken(mContext), type,
				new FetchEntryListener() {

					@Override
					public void setData(Entry entry) {
						if (entry instanceof SubscribeOrderList) {
							doAfterFetchSubscribeList(
									(SubscribeOrderList) entry, true);
						} else {
	...
	protected void doAfterFetchSubscribeList(SubscribeOrderList subscribeList,
		...
		// NOTE 通知刷新栏目列表
		ObserverItem catListObserver = new ObserverItem(
				MainProcessObservable.SET_DATA_TO_COLUMN, new Entry());
		CommonApplication.mainProcessObservable
				.notifyProcessChange(catListObserver);

		// NOE 通知显示IndexViewPager
		ObserverItem indexPagerObserver = new ObserverItem(
				MainProcessObservable.SHOW_INDEX_PAGER, new Entry());
		CommonApplication.mainProcessObservable
				.notifyProcessChange(indexPagerObserver);
	...
	protected void doAfterFecthShiye(TagArticleList articleList, boolean success) {
		if (!success)
			return;
		ObserverItem shiyeObserver = new ObserverItem(
				MainProcessObservable.SET_DATA_TO_SHIYE, articleList);
		CommonApplication.mainProcessObservable
				.notifyProcessChange(shiyeObserver);
	}
