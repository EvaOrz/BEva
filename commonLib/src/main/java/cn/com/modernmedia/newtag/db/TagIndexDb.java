...
public class TagIndexDb extends TagDbListenerImplement {
	...
	private static final String SPECIAL_TAG = "special_tag";//10
	...
	public synchronized void addEntry(Entry entry) {
		...
		Map<Integer, List<ArticleItem>> map = articleList.getMap();
		if (map.isEmpty())
			return;
		...
		try {
			...
			for (ArticleItem item : articleList.getArticleList()) {
				ContentValues cv = createContentValues(appid, columnJson, item,
						viewbygroup, tagName, -1, link, SUBSCRIBE_TOP_ARTICLE);
				db.insert(TABLE_NAME, null, cv);
			}
			db.setTransactionSuccessful();
			db.endTransaction();
