...
public class TagArticleListDb extends TagDbListenerImplement {
	...
	public TagArticleList getTagSearch(String search){
		SQLiteDatabase db = this.getReadableDatabase();
		TagArticleList tagArticleList = new TagArticleList();
		Cursor cursor =null;
		try {
			String sql = "SELECT * FROM " + TABLE_NAME + " WHERE " + VALUE + " LIKE '%" + search + "%'";
			cursor = db.rawQuery(sql, null);

			List<ArticleItem> list = tagArticleList.getArticleList();
			while (cursor != null && cursor.moveToNext()) {
				ArticleItem articleItem = new ArticleItem();
				articleItem.setTitle(cursor.getString(5));
				articleItem.setDesc(cursor.getString(6));
