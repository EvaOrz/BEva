...
public class TagInfoListDb extends TagDbListenerImplement {
    ...
    private static final String DESC = "desc";// 21

    private static final String PARENT = "parent";
    private static final String CAT_NAME = "catname";
    private static final String PUBLISH_STATUS = "pubishdstatus";
    private static final String MODIFYUSER = "modifyuser";
    private static final String UPDATETIME = "updatetime";
    private static final String PUBLISHUSER = "publishuser";
    private static final String PUBLISHTIME = "publishtime";
    private static final String DEVICES = "devices";
    private static final String ORIGINL_CATID = "originlCatId";
    private static final String COLUMN_TYPE = "columntype";
    private static final String ADV_UPDATETIME = "advUpdateTime";
    ...
    /**
     * @param o
     * @param parentTagName 父tagName
     * @param tagName       当前tagName
     * @param group         分组
     * @param top
     * @return
     */
    public Entry getEntry(BaseOperate o, String parentTagName, String tagName, String group, String top, TAG_TYPE type) {
        if (!(o instanceof GetTagInfoOperate)) return null;
        GetTagInfoOperate operate = (GetTagInfoOperate) o;
        SQLiteDatabase db = this.getWritableDatabase();
        TagInfoList tagInfoList = operate.getTagInfoList();
        Cursor cursor = null;
        try {
            cursor = db.query(TABLE_NAME, null, getWhere(parentTagName, tagName, group, top, type), null, null, null, ID + " asc");
    ...
    private TagInfo getTagInfo(Cursor cursor, GetTagInfoOperate operate) {
        ...
        if (!TextUtils.isEmpty(columnJson)) {
            try {
                ...
                tagInfo.setColumnProperty(operate.parseColumnProperty(obj, tagInfo.getTagName()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        if (operate != null && operate.getType() == TAG_TYPE.APP_INFO) {
            Log.e( "DbLoseColumn get ",DbLoseColumn.getUpdatetime(mContext));
            tagInfo.getAppProperty().setUpdatetime(DbLoseColumn.getUpdatetime(mContext));
            tagInfo.getAppProperty().setAdvUpdateTime(DbLoseColumn.getAdvUpdatetime(mContext));
        }
        return tagInfo;
    }

    private String getWhere(String parentTagName, String tagName, String group, String top, TAG_TYPE type) {
    ...
    public synchronized void addEntry(Entry entry, TAG_TYPE type) {
        if (!(entry instanceof TagInfoList)) return;
        ...
        try {
            ...
            for (TagInfo info : list) {
                ...
                db.delete(TABLE_NAME, TAG_NAME + "=? and " + PARENT_TAG + "=?", new String[]{info.getTagName(), info.getParent()});
    ...
    public void updateEntry(Entry entry, TAG_TYPE type) {
        if (!(entry instanceof TagInfo)) return;
        TagInfo info = (TagInfo) entry;
        SQLiteDatabase db = this.getWritableDatabase();
        try {
            ContentValues cv = createContentValues(info, type);
            db.update(TABLE_NAME, cv, TAG_NAME + "=? and " + PARENT_TAG + "=?", new String[]{info.getTagName(), info.getParent()});
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
        }
    ...
    public ContentValues createContentValues(Object... obj) throws Exception {
        ...
        cv.put(PHONE_COLUMN_PROPERTY, tagInfo.getColumnProperty().getColumnJson());
        ...
        if (obj.length > 1 && (obj[1] instanceof TAG_TYPE)) {
            ...
            if (type == 0) {
                DbLoseColumn.setUpdatetime(mContext, tagInfo.getAppProperty().getUpdatetime());
                DbLoseColumn.setAdvUpdatetime(mContext, tagInfo.getAppProperty().getAdvUpdateTime());
            }
        }
        return cv;
    }
     ...
     *
     * @param tagName
     * @param parent       为空代表取一级栏目(获取只有二级栏目的时候需要填写parent,因为二级栏目可能会出现多次)
     * @param ignoreParent 忽略父栏目
     * @return
     */
    public TagInfo getTagInfoByName(String tagName, String parent, boolean ignoreParent) {
    ...
    public void clearTable(String parentTagName, String tagName, String group, TAG_TYPE type) {
        SQLiteDatabase db = this.getWritableDatabase();
        try {
            db.delete(TABLE_NAME, getWhere(parentTagName, tagName, group, "", type), null);
