...
public class TagMainProcessHttp extends BaseTagMainProcess {
    ...
    protected void doAfterFecthAppInfo(TagInfoList appInfo, boolean success) {
        if (success) {
            Log.e("TagMainProcessHttp", appInfo.getList().get(0).getAppProperty().getUpdatetime());
            super.doAfterFecthAppInfo(appInfo, success);

            // 下载广告
            if (TextUtils.equals(DataHelper.getAdvUpdateTime(mContext), AppValue.appInfo.getAdvUpdateTime()) && ConstData.IS_DEBUG != 8) {
                // 广告更新时间未变化
    ...
    protected void doAfterFetchAdvList(AdvList advList, boolean success) {
        ...
        if (success) {
            DataHelper.setAdvUpdateTime(mContext, AppValue.appInfo.getAdvUpdateTime());
        }
        ...
        if (TextUtils.equals(DataHelper.getAppUpdateTime(mContext), AppValue.appInfo.getUpdatetime())) {
            ...
            DataHelper.setAppUpdateTime(mContext, AppValue.appInfo.getUpdatetime());
    ...
    protected void doAfterFecthCatList(TagInfoList catList, boolean success, FetchApiType type) {
        if (success) super.doAfterFecthCatList(catList, success, apiType);
        else toEnd(false);
    }
    ...
    private void checkFirstItem() {
        ...
        catItems.addAll(AppValue.ensubscriptColumnList.getColumnTagList(false, false));
        catItems.addAll(AppValue.ensubscriptColumnList.getColumnTagList(true, false));
        if (ConstData.getInitialAppId() == 20 && catItems.size() > 0) catItems.remove(0);
    ...
    private void getArticleList(final TagInfo tagInfo) {
        ...
        TimeCollectUtil.getInstance().addCollectUrl(UrlMaker.getArticlesByTag(mContext,tagInfo,
                "", ""));
        mController.getTagArticles(mContext,tagInfo, "", "", null, new FetchEntryListener() {
    ...
    private void getCatIndex(TagInfo tagInfo) {
        TimeCollectUtil.getInstance().addCollectUrl(UrlMaker.getTagCatIndex(mContext, tagInfo, "", ""));
        mController.getTagIndex(tagInfo, "", "", null, new FetchEntryListener() {
