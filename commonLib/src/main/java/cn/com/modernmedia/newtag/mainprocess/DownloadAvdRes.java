...
public class DownloadAvdRes {
    ...
    public void downloadRunBan() {
        ...
        if (!ParseUtil.mapContainsKey(advMap, AdvList.RU_BAN)) return;
        if (!ParseUtil.listNotNull(advMap.get(AdvList.RU_BAN))) return;

        for (AdvItem item : advMap.get(AdvList.RU_BAN)) {
            if (!ParseUtil.listNotNull(item.getSourceList())) continue;
    ...
    public void downloadSplash(String url) {
        if (ConstData.getAppId() != 20 || TextUtils.isEmpty(url)) return;
        PrintHelper.print("start down splash");
        BreakPointUtil util = new BreakPointUtil(mContext, new DownloadPackageCallBack() {
     ...
     * @param url
     */
    public void downloadBusinessB(String url,BreakPointUtil util) {
