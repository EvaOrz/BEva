...
public class TagMainProcessCache extends BaseTagMainProcess {
    ...
    public TagMainProcessCache(Context context, MainProcessParseCallBack callBack) {
    ...
    protected void doAfterFecthAppInfo(TagInfoList appInfo, boolean success) {
        if (success) {
            ...
            TagProcessManage.getInstance(mContext).showpushOrurl();
    ...
    protected void toEnd(boolean success) {
        ...
        showLoad(false);
        if (callBack != null) callBack.afterFetchCache(success);
    }
