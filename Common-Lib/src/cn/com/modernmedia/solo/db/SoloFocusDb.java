...
public class SoloFocusDb extends SQLiteOpenHelper {
	 ...
	 * @return
	 */
	public List<ArticleItem> getArticleItem(int parentId, int position) {
		SQLiteDatabase db = this.getReadableDatabase();
		GetCatIndexOperate operate = new GetCatIndexOperate(mContext, parentId
				+ "", "0", "0", CommonApplication.soloColumn, position);
		...
		if (ParseUtil.mapContainsKey(index.getSoloMap(), index.getFullKeyTag())
				&& ParseUtil.mapContainsKey(
						index.getSoloMap().get(index.getFullKeyTag()), 1))
			return index.getSoloMap().get(index.getFullKeyTag()).get(1);
	 ...
	 */
	public void clearTable(int catId) {
		SQLiteDatabase db = this.getWritableDatabase();
		try {
			db.delete(TABLE_NAME, PARENTID + "=?", new String[] { catId + "" });
