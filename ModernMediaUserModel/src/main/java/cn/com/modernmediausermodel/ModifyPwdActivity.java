 ...
 * 
 */
public class ModifyPwdActivity extends SlateBaseActivity implements
		OnClickListener {
	...
	protected void doModifyPwd(String oldPwd, String newPassword) {
		...
		if (UserTools.checkPasswordFormat(mContext, newPassword)
				&& user != null) {
			showLoadingDialog(true);
			mController.modifyUserPassword(user.getUid(), user.getToken(),
					...
					new UserFetchEntryListener() {
						...
						public void setData(final Entry entry) {
							...
							showToast(TextUtils.isEmpty(toast) ? getString(R.string.msg_modify_pwd_failed)
									: toast);
						}
					});
		}
