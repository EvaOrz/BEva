...
public class CardDetailActivity extends SlateBaseActivity implements OnClickListener {
    public static final String KEY_CARD = "card"; // 卡片数据
    public static final int REQUEST_CODE_COMMENT = 110; // 评论界面request code
    ...
    private void initDataFromBundle() {
        ...
        if (getIntent() != null && getIntent().getExtras() != null) {
            boolean isSingle = getIntent().getExtras().getBoolean(KEY_IS_SINGLE, false);
            if (isSingle) {
    ...
    public void setNavBtnStatus() {
        if (ParseUtil.listNotNull(cardItemList) && cardItemList.size() > index) {
            ...
            if (item.getIsDel() == 1) {
                ...
                String uid = item.getType() == 2 ? item.getFuid() : item.getUid();
    ...
    public void getComments(int commentId, final boolean isGetMore) {
        if (!isGetMore) showLoadingDialog(true);
        ...
        controller.getCardComments(items, commentId, false, new UserFetchEntryListener() {

            @Override
            public void setData(Entry entry) {
                if (!isGetMore) showLoadingDialog(false);
                if (entry instanceof MultiComment) {
                    if (isGetMore) {
                        ...
                        if (ParseUtil.listNotNull(multiComment.getCommentList())) {
                            Comment moreComment = multiComment.getCommentList().get(0);
                            mComments.getCommentList().get(index).getCommentItemList().addAll(moreComment.getCommentItemList());
                            mComments.getUserInfoMap().putAll(multiComment.getUserInfoMap());
                        } else { // 没有更多评论项
                            checkCommonsIsNull();
                            return;
                        }
                    ...
                    if (isGetMore) { // 重新加载当前页的评论列表
                        pagerAdapter.getCurrentCardDetatilItem().setData(mComments.getCommentList().get(index), mComments.getUserInfoMap());
                    } else {
    ...
    private void addComment(CommentItem item) {
        // 当前所在的卡片
        if (mComments.getCommentList().size() <= index) return;
        // 将登录用户自己的信息存入map中
        ...
        pagerAdapter.getCurrentCardDetatilItem().setData(comment, mComments.getUserInfoMap());
    }
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.card_detail_nav_share) { // 分享
            ...
            if (view.getId() == R.id.card_detail_nav_delete) { // 删除
                deleteCard(Tools.getUid(this), cardItemList.get(index).getId() + "");
                finish();
                showToast(R.string.card_delete_success);
    ...
    private void gotoWriteCommentAtivity() {
        if (cardItemList.size() > index) {
            ...
            UserPageTransfer.gotoWriteComment(this, REQUEST_CODE_COMMENT, cardId, false);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            if (requestCode == REQUEST_CODE_COMMENT && data != null) {
                if (data.getExtras().getSerializable(WriteCommentActivity.RETURN_DATA) instanceof CommentItem)
                    addComment((CommentItem) data.getExtras().getSerializable(WriteCommentActivity.RETURN_DATA));
