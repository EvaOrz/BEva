...
public class UserPageTransfer {
    ...
    public static final int GOTO_MY_VIP = 6;// 登录完成进入我的VIP页
    ...
    private static void transfer(Context context, Class<?> cls, Bundle bundle, boolean anim) {
        transfer(context, cls, bundle, false, -1, anim);
    }

    private static void transfer(Context context, Class<?> cls, Bundle bundle, boolean finish, boolean anim) {
     ...
     * @param bundle      传递参数
     * @param finish      是否关闭页面
     * @param requestCode !=-1，startActivityForResult
     * @param rightIn     跳转页面执行右边进入动画
     */
    private static void transfer(Context context, Class<?> cls, Bundle bundle, boolean finish, int requestCode, boolean rightIn) {
        Intent intent = new Intent(context, cls);
        if (bundle != null) intent.putExtras(bundle);
        if (requestCode == -1) context.startActivity(intent);
        else ((Activity) context).startActivityForResult(intent, requestCode);
     ...
     * 跳转至商业笔记页面
     */
    public static void gotoUserCardInfoActivity(Context context, User user, boolean finish) {
        if (isLogin(context)) {
            Bundle bundle = new Bundle();
            bundle.putSerializable(UserCardView.KEY_USER, user);
            transfer(context, UserCardInfoActivity.class, bundle, finish, true);
        } else gotoLoginActivity(context, GOTO_BUSINESS_NOTE);
    }
    ...
    public static void gotoMyHomePageActivity(Context context, boolean finish) {
        if (isLogin(context)) transfer(context, MyHomePageActivity.class, null, finish, true);
        else gotoLoginActivity(context, GOTO_HOME_PAGE);
    }
     ...
     * 跳转至激活兑换码页面
     */
    public static void gotoCodeActivity(Context context, boolean finish) {
        if (isLogin(context)) {
            if (SlateDataHelper.getUserLoginInfo(context).getVip_end_time() > System.currentTimeMillis() / 1000) {
                Tools.showToast(context, R.string.vip_code_hasPay_vip);
            } else if (SlateDataHelper.getUserLoginInfo(context).getLevel() == 1) {// 订阅没到期
                Tools.showToast(context, R.string.vip_code_hasPay);
            } else transfer(context, VipCodeActivity.class, null, finish, true);
        } else gotoLoginActivity(context, 0);
    }
     ...
     * 跳转至VIP（我的VIP 升级 开通）
     */
    public static void gotoVipActivity(Context context, boolean finish) {
        if (isLogin(context)) {

            LogHelper.checkVipOpen(context, "me-create");
            transfer(context, VipOpenActivity.class, null, finish, true);
        } else gotoLoginActivity(context, 0);
    }
     ...
     * 跳转至我的VIP
     */
    public static void gotoMyVipActivity(Context context, boolean finish) {
        transfer(context, MyVipActivity.class, null, finish, true);
    }
    /**
     * 商城那边的跳转逻辑：升级、开通、去我的vip
     */
    public static void gotoShangchengActivity(Context context, boolean finish) {
        if (isLogin(context)) {
            if (SlateDataHelper.getVipLevel(context) > 1){
                gotoMyVipActivity(context,finish);
            }else if(SlateDataHelper.getVipLevel(context) == 1){
                transfer(context, TaocanUpActivity.class, null, finish, true);
            }else {
                LogHelper.checkVipOpen(context, "me-create");
                transfer(context, VipOpenActivity.class, null, finish, true);
            }
     ...
     * @param msg
     */
    public static void gotoMessageActivity(Context context, Message msg, boolean finish) {
        message = msg;
        if (!isLogin(context)) {
     ...
     * 跳转至卡片详情页面
     */
    public static void gotoCardDetailActivity(Context context, Card card, int index) {
        if (card == null || card.getCardItemList().size() <= index) return;
        Bundle bundle = new Bundle();
        bundle.putSerializable(CardDetailActivity.KEY_CARD, card);
        bundle.putInt(CardDetailActivity.KEY_CARD_INDEX, index);
        transfer(context, CardDetailActivity.class, bundle, false, CardDetailActivity.REQUEST_CODE_SELF, true);
    }

    /**
     * 跳转至卡片详情页面(消息页使用)
     */
    public static void gotoCardDetailActivity(Context context, String cardId, boolean finish) {
        Bundle bundle = new Bundle();
     ...
     */
    public static void gotoUserListActivity(Context context, User user, int pageType, boolean finish) {
        Bundle bundle = new Bundle();
        if (user != null) bundle.putSerializable(USER_KEY, user);
        bundle.putInt(RecommendUserView.KEY_PAGE_TYPE, pageType);
        transfer(context, RecommendUserActivity.class, bundle, finish, UserCardView.REQUEST_CODE_USER_LIST, true);
    }
     ...
     * @param isShowToast
     */
    public static void gotoWriteComment(Context context, int requestCode, String cardId, boolean isShowToast) {
        Bundle bundle = new Bundle();
        ...
        transfer(context, WriteCommentActivity.class, bundle, false, requestCode, false);
     ...
     * 跳转至写笔记页面
     */
    public static void gotoWriteCardActivity(final Context context, boolean finish) {
        if (!isLogin(context)) {
            gotoLoginActivityRequest(context, TO_LOGIN_BY_WRITE);
            return;
        }

        transfer(context, WriteNewCardActivity.class, null, finish, AFTER_WRITE_CARD, false);
        ((Activity) context).overridePendingTransition(R.anim.down_in, R.anim.hold);
    }
     ...
     * @param content
     */
    public static void gotoWriteCardActivity(Context context, String content, boolean finish) {
        if (TextUtils.isEmpty(content)) {
            //            ((Activity) context).finish();
            return;
        }
        Bundle bundle = new Bundle();
        bundle.putString(WriteNewCardActivity.KEY_FROM, WriteNewCardActivity.VALUE_SHARE);
    ...
    public static void gotoLoginActivityRequest(Context context, int requestCode) {
        transfer(context, UserConstData.getLoginClass(), null, false, requestCode, true);
    }
     ...
     *
     * @param user
     * @param content  第三放分享内容
     * @param gotoPage 需要跳转至的页面
     */
    public static void afterLogin(Context context, User user, String content, int gotoPage) {
        if (user == null) return;
        ...
        if (!TextUtils.isEmpty(content)) {
            ...
            // 如果是第一次登录，则进入推荐关注界面
            // 3.4 不弹出推荐关注界面
            if (UserDataHelper.getIsFirstUseCoin(context, user.getUserName())) {
                UserDataHelper.saveIsFirstUseCoin(context, user.getUserName(), false);
        ...
        if (UserApplication.userObservable != null) UserApplication.userObservable.setData("");
    }
    ...
    private static boolean checkAfterLogin(Context context, int gotoPage) {
        if (gotoPage == GOTO_HOME_PAGE) {
            ...
            gotoUserCardInfoActivity(context, SlateDataHelper.getUserLoginInfo(context), false);
        } else if (gotoPage == GOTO_MY_FAV) {
            gotoFavoritesActivity(context);
        } else if (gotoPage == GOTO_MY_VIP) {
            if (SlateDataHelper.getUserLoginInfo(context).getLevel() == 2 && SlateDataHelper.getUserLoginInfo(mContext).getVip_end_time() > System.currentTimeMillis() / 1000) {
                gotoMyVipActivity(context, true);
            } else {
                gotoMyHomePageActivity(context, true);
            }
        } else {
     ...
     *
     * @param context
     * @param from     页面跳转起源（1：邮箱注册；2：新浪微博登录；3: QQ登录；4：手机注册；5:微信登录；其它为0）
     * @param content  第三方应用或者本应用分享的内容
     * @param gotoPage 需要跳转的页面
     */
    public static void gotoUserInfoActivity(Context context, int from, String content, String password, int gotoPage) {
        Bundle bundle = new Bundle();
        ...
        transfer(context, UserConstData.getUserInfoClass(), bundle, from != 0, true);
    }
    ...
    public static void gotoMyCoinUseNoticeActivity(Context context) {
        if (isLogin(context)) transfer(context, MyCoinUseNoticeActivity.class, null, false, true);
        else gotoLoginActivity(context, 0);
    }
     ...
     * @param isFromNotice
     */
    public static void gotoMyCoinActivity(Context context, boolean isFinish, boolean isFromNotice) {
        if (isLogin(context)) {
            ...
            //			} else {
            if (!TextUtils.isEmpty(uid)) {
                transfer(context, NewCoinActivity.class, null, isFinish, true);
            }
        } else gotoLoginActivity(context, UserPageTransfer.GOTO_MY_COIN);
    }
