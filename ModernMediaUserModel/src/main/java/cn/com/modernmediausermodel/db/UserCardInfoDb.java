 ...
 * 
 */
public class UserCardInfoDb extends SQLiteOpenHelper {
	public static final String LIMIT = UserConstData.MAX_CARD_ITEM_COUNT + "";
	private static final String DATABASE_NAME = "user_card_info.db";
	...
	private static UserCardInfoDb instance = null;

	private UserCardInfoDb(Context context) {
		super(context, DATABASE_NAME, null, DATABASE_VERSION);
	}

	public static synchronized UserCardInfoDb getInstance(Context context) {
		if (null == instance) {
			instance = new UserCardInfoDb(context);
		}
		return instance;
	}
	...
	public void onCreate(SQLiteDatabase db) {
		...
		helper.addColumn(BIND_UID, "TEXT");// 14
	 ...
	 * @return
	 */
	public Card getCard(String timeLine, String bind_uid) {
		SQLiteDatabase db = this.getReadableDatabase();
		Card card = new Card();
		card.setUid(bind_uid);
		Cursor cursor = null;
		try {
			cursor = db.query(TABLE_NAME, null,
					getSelection(timeLine, bind_uid), null, null, null,
					TIMELINEID + " desc", LIMIT);
	 ...
	 * @return
	 */
	private String getSelection(String timeLine, String bind_uid) {
		String where = BIND_UID + "=" + bind_uid;
		if (timeLine.equals("0"))
			return where;
		return where + " and " + TIMELINEID + "<" + timeLine;
	}
	 ...
	 * @param card
	 */
	public void addCardItem(Card card, String bind_uid) {
		...
		try {
			...
			for (CardItem item : card.getCardItemList()) {
				ContentValues cv = createContentValues(item, bind_uid);
				String select = BIND_UID + "=? and " + CARD_ID + "=?";
				String[] args = new String[] { bind_uid, item.getId() };
	...
	private ContentValues createContentValues(CardItem item, String bind_uid) {
	 ...
	 */
	public void clearTable(String bind_uid) {
		SQLiteDatabase db = this.getWritableDatabase();
		try {
			db.delete(TABLE_NAME, BIND_UID + "=?", new String[] { bind_uid });
