...
import cn.com.modernmedia.util.sina.exception.WeiboHttpException;
...
public class HttpManager {
	 ...
	 *            存放参数的容器
	 * @param file
	 *            文件路径，如果 是发送带有照片的微博的话，此参数为图片在 SdCard 里的绝对路径
	 * 
	 * @return 返回响应结果
	 ...
	 *            存放参数的容器
	 * @param file
	 *            文件路径，如果是发送带有照片的微博的话，此参数为图片在 SdCard 里的绝对路径
	 * 
	 * @return 返回响应结果
	 * @throws WeiboException
	 *             如果发生错误，则以该异常抛出
	 */
	public static String uploadFile(String url, String method,
			WeiboParameters params, String file) throws WeiboException {
		String result = "";
		try {
			HttpClient client = getNewHttpClient();
	 ...
	 *             如果发生错误，则以该异常抛出
	 */
	public static ByteArrayOutputStream openUrl4Binary(Context context,
			String url, String method, WeiboParameters params, String file)
			throws WeiboException {
		...
		try {
			...
			if (method.equals(HTTPMETHOD_GET)) {
				...
				if (!TextUtils.isEmpty(file)) {
					...
					post.setHeader("Content-Type", MULTIPART_FORM_DATA
							+ "; boundary=" + BOUNDARY);
					Utility.UploadImageUtils.revitionPostImageSize(context,
							file);
					imageContentToUpload(bos, file);
				} else {
					if (_contentType != null) {
	 ...
	 * @return
	 */
	private static ByteArrayOutputStream readBytesFromHttpResponse(
